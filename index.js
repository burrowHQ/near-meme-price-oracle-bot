...
const computeMeMe = (dependencyTokenId, memeTokenId, memeTokenDecimals, memePoolId, {memeTokenFractionDigits = 12, ignoreFees = true } = {}) => {
  return {
    ...
    computeCall: async (dependencyPrice) => {
      ...
      try {
        const pool = await near.NearView(nearConfig.refContractName, "get_pool", { pool_id: memePoolId })
        const tokenInIndex = pool.token_account_ids.findIndex((id) => id == memeTokenId);
        const tokenOutIndex = pool.token_account_ids.findIndex((id) => id == dependencyTokenId);
        if (tokenInIndex == -1 || tokenOutIndex == -1) {
          console.error(`Invalid ${memeTokenId} pool id`);
          return null;
        }
        const balanceIn = pool.amounts[tokenInIndex];
        const balanceOut = pool.amounts[tokenOutIndex];

        const amountIn = Big(10).pow(memeTokenDecimals);
        const fee = ignoreFees ? 0 : pool.total_fee;
        const amountWithFee = amountIn.mul(Big(10000 - fee));
        const dependencyTokenAmount = amountWithFee
          .mul(balanceOut)
        ...
        console.log(`${memeTokenId} compute price failed:`, e);
        return null;
      }
    },
  };
...
const TestnetComputeCoins = {
  "token.dev-burrow.testnet": computeMeMe("wrap.testnet", "token.dev-burrow.testnet", 18, 759),
  "lonk.fakes.testnet": computeMeMe("wrap.testnet", "lonk.fakes.testnet", 8, 716),
  "willa.fakes.testnet": computeMeMe("wrap.testnet", "willa.fakes.testnet", 18, 459),
  //   "weth.fakes.testnet": {
